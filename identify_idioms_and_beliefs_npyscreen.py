...
with open("expanded_my_words.txt", "r") as file:
...
for filename in os.listdir('./Sandbox/JSON_sed'):
    ...
    #####################################################################################################################################
    ranked_beliefs_data = rank_beliefs(json_data["text"])
...
class JSONFileSelector(npyscreen.ActionFormV2):
    def create(self):
        self.file_list = self.add(npyscreen.TitleSelectOne, max_height=57, name="Select a JSON file to process:", values=[], scroll_exit=True)
#        self.exit_button = self.add(npyscreen.ButtonPress, name="Exit", when_pressed_function=self.on_exit)

    def beforeEditing(self):
        json_folder = './Sandbox/CLEAN_UP'  # Replace with your JSON files folder path
        json_files = get_json_files(json_folder)
        self.file_list.values = [os.path.join(json_folder, file) for file in json_files]
        self.file_list.display()

    def on_ok(self):
        selected_file_index = self.file_list.value[0]
        selected_file_path = self.file_list.values[selected_file_index]
        self.parentApp.getForm('OUTPUT').set_file_path(selected_file_path)
        self.parentApp.switchForm('OUTPUT')
...
class JSONOutput(npyscreen.ActionFormV2):
    def create(self):
        self.output = self.add(npyscreen.Pager, max_height=57, name="Output:", values=[], scroll_exit=True)

    def set_file_path(self, file_path):
        self.file_path = file_path

    def beforeEditing(self):
        output_text = read_json_file_and_call_function(self.file_path)

        # Wrap the lines manually
        wrapped_output = []
        for line in output_text.split('\n'):
            wrapped_output.extend(textwrap.wrap(line, width=self.output.width-1))

        self.output.values = wrapped_output
        self.output.display()
...
class JSONFileMenuApp(npyscreen.NPSAppManaged):
    def onStart(self):
        self.addForm('MAIN', JSONFileSelector)
        self.addForm('OUTPUT', JSONOutput)


def generate_file_name():
    now = datetime.now()
    date_time = now.strftime("%Y%m%d_%H%M%S")
    file_name = f"./text_results_output/identify_idioms_and_beliefs_{date_time}.json"
    return file_name

filename = generate_file_name()

def write_and_print_to_terminal(file, text):
    print(text)
    for line in text.split('\n'):
        file.write(line + '\n')
    file.flush()

def read_json_file_and_call_function(file_path):
    with open(file_path, 'r') as json_file:
        ...
        date_match = re.search(date_regex, file_path)
        time_match = re.search(time_regex, file_path)
        ...
        result_text.append(f"TIME: {time}")

        ranked_beliefs_data = rank_beliefs(json_data["text"])

        result_text.append(f'BELIEFS:\n {json.dumps(ranked_beliefs_data, indent=4)}')
        ...
        # Print the word counts
        for word, count in counts:
            result_text.append(f"{word}: {count}")
        
        with open(filename, 'w') as file:
            write_and_print_to_terminal(file, '\n'.join(result_text))
